...
def home(request):
    if request.session.get('usuario'):
        ...
        livros_emprestadar = Livros.objects.filter(usuario=usuario).filter(emprestado=False)

        livros_devolver = Livros.objects.filter(usuario=usuario).filter(emprestado=True)
        ...
        return render(request,'home.html', {'livros' : livros, 'usuario_logado': request.session.get('usuario'),
                                            ...
                                            'usuarios':usuarios,'livros_emprestadar':livros_emprestadar,'total_livros':total_livros,'livros_devolver':livros_devolver})
    else:
    ...
    if request.session.get('usuario'):
        ...
        if request.session.get('usuario') == livro.usuario.id:
            ...
            return render(request, 'ver_livro.html',{'livro': livro, 'categoria_livro': categoria_livro, 'emprestimos': emprestimos,
                                                     ...
                                                     'id_livro': id, 'usuarios':usuarios,'livros_emprestadar':livros_emprestadar,
                                                     'livros_devolver':livros_devolver })
        else:
...
def cadastrar_devolucao(requst):
    id=requst.POST.get('id_livro_devolver')
...
def seus_emprestimos(request):
    ...
    return render(request, 'seus_emprestimos.html',{'usuario_logado':request.session['usuario'],
                            'emprestimos': emprestimos, 'form': form, 'form_categoria': form_categoria})
